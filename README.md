...
- amae-koromo에서 발급받은 API 키의 token 값만 `AMAE_API_TOKEN` 환경 변수에 설정합니다. 보호된 최근 대국 요청에 `Authorization: Bearer <token>`을 적용하며, 서버 요청 속도는 최대 1 QPS로 제한됩니다. 키가 없거나 최근 대국 조회가 차단되면 기존 캐시를 보존하고 순위율 통계를 대신 표시합니다.
